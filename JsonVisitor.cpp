...
int main() {
    // prepare dummy data
    nlohmann::json user = {{"name", "hakan gedek"},
                           {"id", 1},
                           {"email", "hgedek@pm.me"}};
    product = {{"name", "thinkpad p3"},
               {"price", 888},
               {"stock", 33},
               {"id", 311}};
    order = {{"products", {31}},
    ...
    using Dto_t = std::variant<UserDto, ProductDto, OrderDto>;

    std::vector<Dto_t> dtos = {usrDto, proDto, ordDto};
    ...
    for (auto const& dto : dtos) {
        std::visit(
            [&](const auto& data) {
                ...
                if constexpr (std::same_as<T, UserDto>) {
                    std::println("User => Id: {} Name: {} Email: {}", data.Id,
